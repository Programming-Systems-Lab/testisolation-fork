...
public class EclipseTestRunner implements TestListener {
	...
	public static int run(String[] args) throws IOException {
		...
		if (testPluginsNames != null && classesNames != null) {
			...
			for (String oneClassName : suiteClasses) {
				...
				// WL added conditional for printing tests
				if (outputFile != null) {
	...
	// WL prints the tests from runner into a file and the console.
	private static void printTests(String testsFile, EclipseTestRunner runner) {
		List<Test> tests = runner.fSuite.getTests();
		String output = "";	
		for (Test each : tests) {
			output += each + "\n";			
		}
		try {
			// Create file 
			FileWriter fstream = new FileWriter(testsFile);
		...
		System.out.println(testsFile + " has been successfully written. The content written are: ");
		System.out.println();
