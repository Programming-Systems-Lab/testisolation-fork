...
%\vspace{-3mm}
\IF{$\exists$ Test $\mathit{t} \in ts$ such that $\mathit{lastWrites}$[$t$] $\neq$ $\mathit{lastWriteTS}$[$t$]  }
...
test, if \textit{every global field} it reads
is written by the \textit{same} test as
originally in the default execution order, all tests in
the permutation are guaranteed to
produce the same results. Thus, the permutation can be safely ignored. 
...
field(s), some permutations can be discarded
by checking the tests performing the last write.
