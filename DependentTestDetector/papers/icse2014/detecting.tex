...
and returns the execution results \result{ts}{\env_0}.\\
...
\STATE $\mathit{depTests}$ $\leftarrow$ $\emptyset$
\STATE $\mathit{expectedResults}$ $\leftarrow$ executeTests($T$)\\
\COMMENT{$\mathit{reads}$ and $\mathit{writes}$'s data type: Map$\langle$Test, Set$\langle$Field$\rangle$$\rangle$, which maps a test
to a set of fields.}\\
