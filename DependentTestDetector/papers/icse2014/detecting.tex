...
\begin{figure}[t]
\textbf{Auxiliary methods}:\\
recordFieldAccess($\mathit{t}$): executes a test $\mathit{t}$ and returns the fields it reads and writes.\\
...
\FOR{each $\mathit{ts}$ in kPermutations($\mathit{T}$, $\mathit{k}$)}
	\FOR {each $\mathit{t_i}$ in $\mathit{ts}$ \{ $\mathit{i}$ is the index of $\mathit{t_i}$ in $\mathit{ts}$ \}} 
		...
		\IF {$\mathit{previousWrites} \cap \mathit{reads_{t_i}} \neq \emptyset$}
...
fields that each test reads and writes when executed in isolation
