...
An advantage of this algorithm is that it produces the shortest possbile
...
detects the same number\todo{I don't think that is true if a dependence is
  database- or filesystem-carried.  In that case, the \dependenceaware{}
  algorithm is unsound.  That should be discussed, or the way it is made
  sound should be mentioned.}
of dependent tests
...
other external resources such as a file\todo{Is that implemented?  If so,
  this is too late as a first mention.} is not a dependent test. 
...
\todo{What does it do with them?  Neither the text nor
  Figure~\ref{fig:impralg} say anything about that.}
Then, it runs the algorithm shown in Figure~\ref{fig:impralg}.\todo{What is
...
field(s), some permutations can be ignored by checking\todo{What does
  ``check'' mean?  Also, is it implemented?  Change ``can'' to ``is'' or
  ``could be''.}
