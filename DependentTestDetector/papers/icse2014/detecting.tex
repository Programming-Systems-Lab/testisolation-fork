...
\STATE $\langle {reads}_t, {writes}_t\rangle$ $\leftarrow$ recordFieldAccess($\langle t \rangle$)\\ 
\ENDFOR

\STATE $\mathit{result}$ $\leftarrow$ $\emptyset$ \\
\FOR{each $\mathit{ts}$ in kPermutations($\mathit{T}$, $\mathit{k}$)}
	\FOR {each $t_i$ in $ts$ \{ $i$ is the position of $t_i$ in $ts$ \}} 
		\STATE $previousWrites$ $\leftarrow$ $\bigcup_{j < i}  {writes}_{t_j} $ \\
		\IF {$previousWrites \cap reads_{t_i} \neq \emptyset$}
			\STATE $result \leftarrow results \cup ts$
		\ENDIF
	\ENDFOR
...
a permutation can safely be ignored if all global variables a test accesses
are the same as a previously executed permutation. 
...
fields that each test reads and writes in isolation (line 1).
When generating all possible test permutations
up to length $k$, the algorithm ensures
\textit{all} fields \textit{each} test (in the generated permutation)
may read are not written by any previous test. If so, all tests in the permutation
must produce the same results as execution in isolation, and the algorithm discards
...
field(s), some permutations can be avoided by considering their
order.
...
%  LocalWords:  getPossibleExecOrder numtrials lastWriteTS
