...
Section~\ref{sec:reversing} describes a heuristic algorithm
that executes all the tests of a suite in a reverse order.
...
Figures~\ref{fig:reversingalgorithm},~\ref{fig:randalgorithm},~\ref{fig:exhaustivealgorithm},
and~\ref{fig:impralg}.
}
...
\vspace{-3mm}
\caption {The reversing algorithm to detect dependent tests.
...
\subsection{Reversing Algorithm}
\label{sec:reversing}
...
Figure~\ref{fig:reversingalgorithm} instantiates the
base algorithm (Figure~\ref{fig:basealgorithm}) for the
heuristic algorithm that reverses the original test execution
order.
...
base algorithm (Figure~\ref{fig:basealgorithm}) for the
randomized algorithm by randomizing the original test execution
order (line 2).
