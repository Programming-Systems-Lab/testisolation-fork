...
field and file access information\todo{used how?}, after being executed
...
since \todo{The remainder of this sentence is not clear:}a read access to a static field may mutate objects
...
classes.\todo{How is that immutability information used?}
...
list of ``dependence-free''\todo{Try to think of a better (more descriptive
  or informative) name for these} fields (e.g., a static field for
logging or counting), which can never be the root cause of manifest test
dependence.\todo{Explain or reword ``can never be the root cause''.}
