...
it identifies \textit{more} dependent tests, and the
dependent tests found by the randomized algorithm are a
superset of those detected by the other two algorithms.
...
Second, more importantly, there is no clear stopping criterion
...
tests exist in a test suite and \ourtool's recall.  Giving a reasonable estimation is our future work. Fourth, we only assessed the
impact of dependent tests on five test prioritization
techniques, and we only evaluated the impact of dependent
tests on prioritizing unit tests.
Using other test prioritization techniques and
other types of tests (e.g., integration tests
or system tests) might achieve different results. 
...
of the number of dependent tests it can detect. Testers
