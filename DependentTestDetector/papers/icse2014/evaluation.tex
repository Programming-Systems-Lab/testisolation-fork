...
dependent tests are previously-unknown) and 1311
...
did not track field access within the \CodeIn{java.security} package,
...
impact of dependent tests to two well-known
downstream testing techniques: test
selection and test prioritization.
We count the number of dependent tests in the
4 human-written test suites (Table~\ref{tab:results}) that can
affect the results of these two techniques.
...
\todo{Add a sentence like: ``the problem is np-complete,
even a smart algorithm cannot achieve great results.'' somewhere}
%However, it does not
