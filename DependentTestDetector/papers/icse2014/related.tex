...
Further, a test may non-deterministically succeed/fail without being
...
versions)~\cite{Orso:2004:SRT}, without considering test dependence.
%
A stronger assumption than determinism is the Controlled Regression
Testing Assumption~\cite{Rothermel:1996:ART}.  It forbids porting to another system,
...
mention it explicitly.  The authors state that the assumption is ``not
...
%JUnit 4.x). 
The latest release 4.11 of JUnit supports
...
known, and
aims to improve existing test prioritization techniques
