...
Further, our experimental results show that programmers often do not
use them properly and introduce dependent tests. 

Only a few tools explicitly 
allows developers to annotate dependent tests and
...
test prioritization techniques that consider potential
test dependencies. However, with a significantly different focus,
their work aims to improve existing test prioritization techniques
and make them produce a test ordering that preserves the test dependencies.
Compared to our work, their work does
\textit{not} give a formal definition to test dependence nor
provide evidence whether test dependence arises in practice, rather,
their work \textit{assumes} the some dependencies between tests
(i.e., a test should be executed before another) are known.
Further, they did not provide any empiricial results
about the repercussions
of dependent tests, how to detect dependent tests, and
the impact of dependent tests on existing test prioritization techniques.
...
JCrasher~\cite{Csallner:2004} does this, as do some SIR applications.
%\todo{Check the math.}
