...
A possible way to eliminate test dependence is
completely re-initializing the environment (e.g., global variables,
heaps, files, operating system services, etc.) before executing a test (or
generating a new test in a test generator~\cite{Csallner:2004}).
However, such an approach can be prohibitively expensive
and thus becomes less useful in practice, since a unit test is often small
and its execution cost is much cheaper than the cost of envrioment initialization.
As shown in our experiments (Table~\ref{tab:results}), 
executing each test separately in a separate JVM 
(see the ``Analysis Cost (seconds) | Exhaustive | $k$=1'' column) 
can introduce as much as 130X runtime overhead than executing each test
in a shared JVM (see the ``Analysis Cost (seconds) | Randomized'' column).
