...
test dependence. However, with a significantly different focus,
...
\textit{not} give a formal definition to test dependence,
%provide evidence whether test dependence arises in practice,
rather, their work \textit{assumes} some dependencies between tests
(which is represented as partial orderings, such as one test should be executed before another) are known.
Further, they did not provide any empiricial results
to show dependent tests exist in practice,
the repercussions of dependent tests, how to detect dependent tests, and
