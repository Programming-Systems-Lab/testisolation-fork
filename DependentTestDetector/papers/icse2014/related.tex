...
Another possible way to eliminate test dependence is
completely re-initializing the environment before executing a test (or
generating a new test in a test generator~\cite{Csallner:2004}).
However, such an approach can be prohibitively expensive
and becomes less useful in practice, since a unit test is often small
and its execution cost is often much cheaper than re-initializing the envrioment.
As shown in our experiments (Table~\ref{tab:results}), 
executing each test separately in a fresh JVM 
(see the ``Analysis Cost (seconds) | Exhaustive | $k$=1'' column) 
introduces a 130X runtime overhead than executing all tests
in a single JVM (see the ``Analysis Cost (seconds) | Randomized'' column).
