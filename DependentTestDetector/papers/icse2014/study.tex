...
\CodeIn{argName}. CLI should set value \CodeIn{"arg"} to
the static variable \CodeIn{argName} before
its clients instantiate an \CodeIn{OptionBuilder} object.
However, the CLI implementation in Figure~\ref{fig:option_builder} only sets the value
after the clients call method \CodeIn{reset()}. 
%Otherwise, \CodeIn{argName}'s
