...
in the issue report.\todo{Why is the size of the reported sequence
...
49 such dependent tests.\todo{An issue tracking system does not contain
...
\todo{The following paragraph is a candidate for shortening.}
...
to its default value \CodeIn{"arg"}\todo{I'm confused by this terminology.
  According to Java language semantics, the default value of the static
  variable is null.  Should the field initializer have been changed or removed?} by CLI's clients via calling
...
can reveal this fault by directly initializing\todo{I don't understand
  ``directly initializing''.  Should that be replaced by ``intantiating''?}
...
may achieve a high return\todo{I'm not sure what ``achieve a high return'' means} by focusing on global variables.
