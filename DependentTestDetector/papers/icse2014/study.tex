...
In our study, we also found 1 test depending on itself:
running the test twice produces different results than running it once,
since this test has side-effect on the database it reads.
We count this special case separately in the ``Self'' column
of Table~\ref{tab:studyresults}.

For \unum dependent tests, the number of involved tests
is unknown, since the relevant information is not described
...
The root cause for the remaining 23 (25\%) tests is not described clearly
...
In some cases, dependent tests are intentional, developers are aware
...
but only 38 (39\%) of them got fixed by developers. Among
the 38 fixed dependent tests, only 6 fixes are
...
easier and more convenient to write the test code. When a dependent
test is reported, developers simply add comments to the test code
to document the existing dependency. This applies to 49 (84\%) out
of the \unfixed unfixed tests. For the rest 9 (16\%) unfixed tests,
developers thought they were not important enough given their limited
development time, and simply closed the issue report without taking
any action.
...
fixed on the program code, developers changed the relevant
code that performs static variable initialization, to ensure
the dependent tests will not read an undesired value.
...
from the test suite, and the rest 1 (3\%) test was merged it with its
initializing test.
...
root cause: in SWT, only one global \texttt{Display}
...
Every dependent test is reported when the
...
by improper static variable accesses. This implies a technique
