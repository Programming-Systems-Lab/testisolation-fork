...
We investigated the validity of this unverified conventional wisdom,
...
the test's pass/fail result (determined by its existing testing
...
\item \textbf{Formalization.} We formalize test dependence
  ...
  % one that prunes the search space using dynamic analyses
  ...
  \item \textbf{Evaluation.} We implemented our algorithms in a 
  tool called \ourtool (Section~\ref{sec:impl})\footnote{\ourtool
  ``exceeded expectations'' of the ISSTA 2014 artifact evaluation
  committee.
  It is
 publicly available at \url{https://testisolation.googlecode.com/}.  
  }.
...
%  LocalWords:  Kapfhammer Soffa subsequence SWT CLI NUM dependences ICST

%  LocalWords:  teardown TSE Anecdotally
