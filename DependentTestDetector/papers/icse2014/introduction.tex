...
an \textit{order-dependent} test (in the context of this suite), since its result depends on
...
test parallelization~\cite{Misailovic:2007}.
...
We adopt the results of the default, usually
...
program; but the answer to this is trivially ``yes''.)


This paper uses \textit{dependent test} as a shorthand for
\textit{manifest order-dependent test} for brevity
unless explicitly noted.
%
A single test may consist of setup and teardown
code, multiple statements, and multiple assertions (the oracle) distributed
through the test.
...
major consequences of the fact that a dependent test gives different
results depending on when it is executing during testing.

\textbf{(1)}
In some cases, dependent tests can
...
\textbf{(2)}
More commonly, test dependences can lead to \emph{spurious bug reports}.
...
\item \textbf{Formalization.} We formalize test dependence
  ...
  unit tests in \subjnum real-world subject programs (and even more in
  automatically-generated tests).  The developers confirmed all of these as
  undesired.
