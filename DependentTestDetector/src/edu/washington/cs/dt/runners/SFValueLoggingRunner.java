...
public class SFValueLoggingRunner extends AbstractTestRunner {
	
	private float executionTime = 0.0f;

	public SFValueLoggingRunner(List<String> tests) {
		super(tests);
	}
	
	public SFValueLoggingRunner(String fileName) {
		super(fileName);
	}
	...
	public TestExecResults run() {
		...
		for(String test : super.junitTestList) {
			Map<String, OneTestExecResult> singleRun = TestExecUtils.executeTestsInFreshJVMAndLogSFs(super.getClassPath(),
